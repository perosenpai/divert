...
import {updateMember} from '../../utils/APIRoutes'
import {updateTeamMemberEmail} from '../../utils/APIRoutes'
import {hideLoader, showLoader} from '../../App.js'

let user = localStorage.getItem("divert-user");
user = JSON.parse(user);

export const reloadPageUponUpdate = async (userMember) =>{
  showLoader()

  let allUsers = await axios.get(allUsersRoute);

  let foundIt = await allUsers.data.find(x => x.email === userMember.email);
  let foundItById = await allUsers.data.find(x => x._id === userMember.id);

      if(foundItById){ 
        if(foundItById.email !== userMember.email){
          const {data} = await axios.put(updateTeamMemberEmail, {
            "id": foundItById._id,
            "avatar": foundItById.avatar,
            "email": foundItById.email,
            "oldEmail": userMember.email,
            "status": 2,
            "teamId": teamId
            }
          );
      ...
      if(foundIt){
        if(foundIt._id !== userMember.id){
          const {data} = await axios.put(updateMember, {
              "id": foundIt._id,
              "avatar": foundIt.avatar,
              "email": foundIt.email,
              "status": 2,
              "teamId": teamId
            }
        ...
        if(foundIt.avatar !== userMember.avatar){
          const {data} = await axios.put(updateMember, {
              "id": foundIt._id,
              "avatar": foundIt.avatar,
              "email": foundIt.email,
      ...
      current.Users.forEach(async(user) =>{
        userElement.innerHTML += `
                  <div class="user-container" style="padding: 10px; width: 300px">
                    ...
                    await reloadPageUponUpdate(user)
        return userElement;
      });
