...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(handleValidation()){
      ...
      if(data.status === true){
        localStorage.setItem('divert-user', JSON.stringify(data.data));
        navigate("/");
            ...
            <Col className="col-lg-6 column-first shadow-lg">
              <div className="p-5">
                <div className="text-left" style={{ textAlign: 'left !important' }}>
                  ...
                  <h4 className="mb-4" style={{ marginTop: '30px'}}>Welcome to Divert.</h4>
                </div>
              <Form className="user" onSubmit={(event) => handleSubmit(event)}>
                ...
                <button id="button-login"className="btn btn-primary btn-block text-white btn-user" type="submit">Login</button>
                <hr />  
                <span>Don't have an Account? <Link to="/register" id="register-hover"><b>Register here.</b></Link></span>
              </Form>
              </div>
            </Col>
